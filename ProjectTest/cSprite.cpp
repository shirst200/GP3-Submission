...
void cSprite::updateAnimated(int noAlong, int noUp) 							// update the position etc of the sprite
{
	D3DXMatrixTransformation2D(&spriteTransformMatrix,
		&cSprite::getSpriteCentre(),
		cSprite::getSpriteScalingRotation(),
		...
		cSprite::getSpriteRotation(),
		&cSprite::getSpritePos2D());
	cSprite::setBoundingAnimatedRect(&boundingRect,noAlong,noUp);
}
/*
=================
...
void cSprite::setBoundingAnimatedRect(RECT* pRect,int noAlong,int noUp)
{
	D3DXVECTOR3 sPos = getSpritePos();
	int sImageWidth = (getSTWidth()/noAlong);
	int sImageHeight = (getSTHeight()/noUp);
	SetRect(pRect, (int)sPos.x, (int)sPos.y, (int)(sImageWidth+sPos.x), (int)(sImageHeight+sPos.y));
}
/*
=================
